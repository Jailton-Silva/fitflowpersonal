...
import { User, Cake, Ruler, Weight, Dumbbell, Shield, Phone, Activity, History, Calendar as CalendarIcon, Utensils, ArrowLeft, Trophy, Upload, Loader2 } from "lucide-react";
...
export default function StudentPortalClient({ student, initialWorkouts = [], initialMeasurements = [] }: StudentPortalClientProps) {
    ...
    // Local state for avatar to provide immediate feedback
    ...
    const filteredWorkouts = useMemo(() => {
        ...
        return initialWorkouts.filter(w => 
            !lowerCaseFilter || w.name.toLowerCase().includes(lowerCaseFilter)
        );
        ...
        startUploadTransition(async () => {
            const result = await uploadStudentAvatar(student.id, formData);
             if (result?.error) {
                ...
                setAvatarUrl(student.avatar_url); // Revert on error
            } else {
                toast({ title: "Sucesso!", description: "Sua foto de perfil foi atualizada."});
                // The new URL will be re-fetched on next page load, but optimistic one stays for now
            }
        });
    }
            ...
            <div className="max-w-4xl mx-auto space-y-6">
                <div className="flex flex-col sm:flex-row gap-6 items-center bg-card p-6 rounded-lg shadow-sm">
                    ...
                    <div className="flex-1 space-y-1 text-center sm:text-left">
                        ...
                        <div className="flex items-center justify-center sm:justify-start text-sm text-muted-foreground pt-1 gap-4 flex-wrap">
                 ...
                 <Card>
                    ...
                    <CardContent>
                        {filteredWorkouts.length > 0 ? (
                            ...
                            <p className="text-muted-foreground text-center py-4">Nenhum plano de treino encontrado para sua busca.</p>
                        )}
                    </CardContent>
                </Card>
