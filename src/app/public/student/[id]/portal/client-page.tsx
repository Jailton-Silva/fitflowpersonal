...
import { Activity, Dumbbell, History, PlusCircle, Calendar as CalendarIcon, User, Cake, Ruler, Weight, Shield, Phone, Edit, ArrowLeft, AreaChart, BarChart } from "lucide-react";
...
export default function StudentPortalClient({ student, initialWorkouts = [], initialMeasurements = [], initialSessions = [] }: StudentPortalClientProps) {
    ...
    useEffect(() => {
        if (initialSessions && initialSessions.length > 0) {
            ...
            setLastActivity(format(new Date(mostRecentSession.started_at), "dd/MM/yyyy 'às' HH:mm", { locale: ptBR }));
        }
    }, [initialSessions]);
                ...
                <div className="max-w-4xl mx-auto space-y-6">
                    ...
                    <header className="flex flex-col sm:flex-row gap-6 items-start">
                        ...
                        <div className="flex-1 space-y-1">
                            ...
                            {lastActivity && (
                                <p className="text-sm text-muted-foreground">Última atividade: {lastActivity}</p>
                            )}
                        </div>
                    </header>
                    ...
                    <Card>
                        <CardHeader><CardTitle className="text-lg font-headline flex items-center"><BarChart className="mr-2" /> Gráfico de Evolução Física</CardTitle></CardHeader>
