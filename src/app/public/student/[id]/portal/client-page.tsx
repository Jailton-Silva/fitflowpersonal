...
import { Upload, Loader2, Dumbbell, Activity, History, Calendar, Ruler, Weight, Shield, Phone, Edit, User, Cake } from "lucide-react";
    ...
    const [lastActivity, setLastActivity] = useState<string | null>(null);
     ...
     useEffect(() => {
        if (initialSessions.length > 0) {
            const lastSession = initialSessions[0];
            const date = lastSession.completed_at || lastSession.started_at;
            setLastActivity(format(new Date(date), "dd/MM/yyyy 'às' HH:mm"));
        }
    }, [initialSessions]);
             ...
             <main className="flex-1 py-8 px-4 sm:px-6 lg:px-8">
                 <div className="max-w-4xl mx-auto space-y-6">
                    <header className="flex flex-col sm:flex-row gap-6 items-center">
                        ...
                        <div className="flex-1 space-y-2 text-center sm:text-left">
                           <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                             ...
                             <p className="text-sm text-muted-foreground">Última atividade: {lastActivity || 'Nenhuma registrada'}</p>
                        </div>
                    </header>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        ...
                        <Card>
                            <CardHeader><CardTitle className="text-lg font-headline flex items-center"><Dumbbell className="mr-2"/> Objetivos</CardTitle></CardHeader>
                            <CardContent><p className="text-sm">{student.goals || "Nenhum objetivo definido."}</p></CardContent>
                        </Card>
                        <Card>
                            <CardHeader><CardTitle className="text-lg font-headline flex items-center"><Shield className="mr-2"/> Obs. Saúde</CardTitle></CardHeader>
