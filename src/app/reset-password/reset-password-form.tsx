...
export default function ResetPasswordForm() {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    // The user's session is automatically set by the time they reach this page
    // if they've come from the password recovery email link.
    const { error } = await supabase.auth.updateUser({ password: data.password });

    if (error) {
      toast({ 
        title: 'Erro ao Alterar Senha', 
        description: 'Não foi possível atualizar a senha. O link pode ter expirado ou ser inválido. Por favor, tente novamente.', 
        variant: 'destructive' 
      });
